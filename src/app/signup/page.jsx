...
export default function SignUpPage() {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      const res = await fetch("https://nest-solution-backend.vercel.app/api/auth/signup", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username, email, password }),
      });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 to-gray-700 px-4 md:px-0">
      <Toaster />
      ...
      <div className="w-full max-w-md bg-white/10 backdrop-blur-xl border border-white/20 shadow-2xl rounded-2xl p-8 animate-fadeIn">
        <h2 className="text-2xl md:text-4xl font-bold text-center font-poppins text-white mb-6">
          Create Account
        </h2>
